    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (l > 0) and (l <= XFaceConstants.cWIDTH) and (m > 0) then
            if LF1.Bitmap[l + m * XFaceConstants.cWIDTH] <> 0 then
              k := k * 2 + 1
            else
              k := k * 2;
        end;
      ...
      case i of
        1 :
          begin
            case j of
              1 : LF.Bitmap[h] := LF.Bitmap[h] xor g_22[k];
              2 : LF.Bitmap[h] := LF.Bitmap[h] xor g_21[k];
              else
                LF.Bitmap[h] := LF.Bitmap[h] xor g_20[k];
            end;
          end;
        2 :
          begin
            case j of
              1 : LF.Bitmap[h] := LF.Bitmap[h] xor g_12[k];
              2 : LF.Bitmap[h] := LF.Bitmap[h] xor g_11[k];
              else
                LF.Bitmap[h] := LF.Bitmap[h] xor g_10[k];
            end;
          end;
        XFaceConstants.cWIDTH - 1 :
          begin
            case j of
              1 : LF.Bitmap[h] := LF.Bitmap[h] xor g_42[k];
              2 : LF.Bitmap[h] := LF.Bitmap[h] xor g_41[k];
              else
                LF.Bitmap[h] := LF.Bitmap[h] xor g_40[k];
            end;
          end;
        XFaceConstants.cWIDTH :
          begin
            case j of
              1 : LF.Bitmap[h] := LF.Bitmap[h] xor g_32[k];
              2 : LF.Bitmap[h] := LF.Bitmap[h] xor g_31[k];
              else
                LF.Bitmap[h] := LF.Bitmap[h] xor g_30[k];
            end;
          end;
        else
          begin
            case j of
              1 : LF.Bitmap[h] := LF.Bitmap[h] xor g_02[k];
              2 : LF.Bitmap[h] := LF.Bitmap[h] xor g_01[k];
              else
                LF.Bitmap[h] := LF.Bitmap[h] xor g_00[k];
            end;
          end;
      end;
    end;
    ...
    begin
      ...
      begin
        if (LByte and c2 <> 0) then
          AXFaceBitMap.Bitmap[t2] := 1
        else
          AXFaceBitMap.Bitmap[t2] := 0;
        c2 := c2 shr 1;
        Inc(t2, 1);
      end;
    end;
...
begin
  ...
  try
    ...
    begin
      if AXFaceBitMap.Bitmap[s] <> 0 then
        LByte := LByte * 2 + 1
      else
        LByte := LByte * 2;
